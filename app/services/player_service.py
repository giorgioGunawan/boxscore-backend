...
class PlayerService:
    """Service for player-related operations with hybrid data provider."""
    ...
    async def get_player_by_id(db: AsyncSession, player_id: int) -> Optional[dict]:
        ...
        # Add metadata
        return DataProvider.add_metadata(response, player)
    ...
    async def get_player_season_averages(
        ...
        """
        Get player's season averages with hybrid data provider pattern.
        
        Priority:
        1. If manual override -> Return local data
        2. If fresh data exists -> Return local data
        3. Try NBA API -> Store and return
        ...
        # If manual override exists, use local data
        if stats and DataProvider.is_manual_override(stats):
            response = _build_season_averages_response(player, stats, season, season_type)
            return DataProvider.add_metadata(response, stats)
        
        # If fresh data exists and not forcing refresh, use local
        if stats and not force_refresh and DataProvider.is_fresh(
            stats.last_api_sync, DataProvider.TTL_PLAYER_STATS
        ):
            response = _build_season_averages_response(player, stats, season, season_type)
            return DataProvider.add_metadata(response, stats)
        ...
        try:
            ...
            print(f"NBA API fetch failed for player stats: {e}")
    ...
    async def get_player_latest_game(
        ...
        """
        Get player's most recent game stats with hybrid data provider pattern.
        ...
        # If manual override exists, use local data
        if game_stats and DataProvider.is_manual_override(game_stats):
            response = await _build_latest_game_response(db, player, game_stats, [])
            if response:
                return DataProvider.add_metadata(response, game_stats)
        ...
        if not game_stats or force_refresh:
            ...
            if game_log:
                ...
                if game:
                    ...
                    game_stats.last_api_sync = datetime.utcnow()
        ...
        response = await _build_latest_game_response(db, player, game_stats, game_log)
        if not response:
            return None
    ...
    @staticmethod
    async def update_player_manual(
        db: AsyncSession,
        player_id: int,
        data: dict,
        reason: str = None
    ) -> Optional[dict]:
        """Update player with manual data override."""
        from app.services.data_provider import set_manual_override
        
        player = await set_manual_override(db, Player, player_id, data, reason)
        if not player:
            return None
    ...
    @staticmethod
    async def update_season_stats_manual(
        db: AsyncSession,
        player_id: int,
        season: str,
        season_type: str,
        ...
        """Update player season stats with manual data override."""
        from app.services.data_provider import set_manual_override
        
        # Find the stats record
        result = await db.execute(
            select(PlayerSeasonStats).where(
                PlayerSeasonStats.player_id == player_id,
                PlayerSeasonStats.season == season,
        ...
        if not stats:
            ...
            from app.services.data_provider import create_manual_record
            stats = await create_manual_record(
    ...
    @staticmethod
    async def clear_player_override(db: AsyncSession, player_id: int) -> Optional[dict]:
        """Clear manual override on player, allowing API sync."""
        from app.services.data_provider import clear_manual_override
        
        player = await clear_manual_override(db, Player, player_id)
        if not player:
            return None
...
def _build_season_averages_response(player: Player, stats: PlayerSeasonStats, season: str, season_type: str) -> dict:
    """Build the season averages response dict."""
    return {
        "player_id": player.id,
...
async def _build_latest_game_response(db: AsyncSession, player: Player, game_stats: PlayerGameStats, game_log: list) -> Optional[dict]:
    """Build the latest game response dict."""
    ...
    # If player has no team, try to determine from the game log
